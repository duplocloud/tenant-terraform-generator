...
func (c *Client) DuploSQSQueueGetV2(tenantID string, fullname string) (*DuploSQSQueue, ClientError) {
	list, err := c.DuploSQSQueueListV2(tenantID)
	if err != nil {
		return nil, err
	}
...
func (c *Client) DuploSQSQueueListV2(tenantID string) (*[]DuploSQSQueue, ClientError) {
	resp := []DuploSQSQueue{}
	err := c.getAPI(
		fmt.Sprintf("DuploSQSQueueListV2(%s)", tenantID),
		fmt.Sprintf("v3/subscriptions/%s/aws/sqs", tenantID),
		&resp,
	)
	return &resp, err
