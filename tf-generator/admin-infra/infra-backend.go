...
type InfraBackend struct {
}

func (ab *InfraBackend) Generate(config *common.Config, client *duplosdk.Client) (*common.TFContext, error) {
	log.Println("[TRACE] <====== Infra backend TF generation started. =====>")
	// create new empty hcl file object
	hclFile := hclwrite.NewEmptyFile()

	// create new file on system
	path := filepath.Join(config.TFCodePath, config.InfraProject, "backend.tf")
	...
	s3BackendBody.SetAttributeValue("key",
		cty.StringVal(config.InfraProject))
